...
class MQTT_Protocol_Parse():
    def parse_fixed_header(self, data: bytes):
        if not data:
        ...
        if control_packet_type == 1: return self.parse_CONNECT(data[1+b_count:(1+b_count)+val])
        elif control_packet_type == 2: return self.parse_CONNACK(data[1+b_count:(1+b_count)+val])
        elif control_packet_type == 3: return self.parse_PUBLISH(data[1+b_count:(1+b_count)+val],flags, val)
        elif control_packet_type == 4: return self.parse_PUBACK(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 5: return self.parse_PUBREC(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 6: return self.parse_PUBREL(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 7: return self.parse_PUBCOMP(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 8: return self.parse_SUBSCRIBE(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 9: return self.parse_SUBACK(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 10: return self.parse_UNSUBSCRIBE(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 11: return self.parse_UNSUBACK(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 12: return self.parse_PINGREQ()
        elif control_packet_type == 13: return self.parse_PINGRESP()
        elif control_packet_type == 14: return self.parse_DISCONNECT(data[1+b_count:(1+b_count)+val], val)
        elif control_packet_type == 15: return self.parse_AUTH(data[1+b_count:(1+b_count)+val], val)

    #Работа с пакетом типа connect
    def parse_CONNECT(self, data: bytes):
        ...
        if reserved_flag != 0: raise ValueError(f'connection reserved flag != 0 {connect_flags}')
        #Keep-alive
        ...
        0x15: lambda data, i: (
                                "Authentication Method", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')].decode('utf-8'),
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
        0x16: lambda data, i: (
                                "Authentication Data", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')],
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
        #will properties
        ...
        0x03: lambda data, i: (
                                "Content Type", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')].decode('utf-8'),
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
        0x08: lambda data, i: (
                                "Response Topic", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')].decode('utf-8'),
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
        0x09: lambda data, i: (
                                "Correlation Data", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')],
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
                            }
        properties = {}
        ...
        if will_flag > 0:
            ...
            while i < will_properties_end:
                ...
                #Может быть несколько User Property
                            ...
                            will_properties[f"User Property {p_us_pr_c}"] = (val1, val2)
                            p_us_pr_c += 1
                else:
                    raise ValueError(f'prop_id = {prop_id} error')
    ...
    def parse_CONNACK(self, data: bytes):
        i = 0
        ...
        0x12: lambda data, i: (
                                "Assigned Client Identifier", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')].decode('utf-8'),
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
        0x13: lambda data, i: ("Server Keep Alive", int.from_bytes(data[i:i+2], 'big'), i + 2),
        0x1A: lambda data, i: (
                                "Response Information", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')].decode('utf-8'),
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
        0x1C: lambda data, i: (
                                "Server Reference", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')].decode('utf-8'),
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
        0x1F: lambda data, i: (
                                "Reason String", data[i+2:i+2+int.from_bytes(data[i:i+2], 'big')].decode('utf-8'),
                                i + 2 + int.from_bytes(data[i:i+2], 'big')
                                ),
    ...
    def parse_PUBLISH(self, data: bytes, flags, remaining_length):
        dup_flags = flags & 0x08
        qos_flags = flags & 0x06
        ...
        i += topic_name_length
        if qos_flags > 0:
            packet_identifier = data[i:i+1]
            i += 2 #packet_identifier = (data[i] << 8) | (data[i+1])
            #Properties
        publish_poperties = {
        0x01: lambda data, i: ("Payload Format Indicator", data[i], i + 1),
        0x02: lambda data, i: ("Message Expiry Interval", int.from_bytes(data[i:i+4], 'big'), i + 4),
    ...
    def parse_PUBACK(self, data: bytes, remaining_length):
        i=0
    ...
    def parse_PUBREC(self, data: bytes, remaining_length):
        i=0
        packet_identifier = (data[i] << 8) | (data[i+1])
        i += 2
        if (remaining_length == 2):
    ...
    def parse_PUBREL(self, data: bytes, remaining_length):
        i=0
        packet_identifier = (data[i] << 8) | (data[i+1])
        i += 2
        if (remaining_length == 2):
    ...
    def parse_PUBCOMP(self, data: bytes, remaining_length):
        i=0
        packet_identifier = (data[i] << 8) | (data[i+1])
        i += 2
        if (remaining_length == 2):
    ...
    def parse_SUBSCRIBE(self, data: bytes, remaining_length):
        i=0
        packet_identifier = (data[i] << 8) | (data[i+1])
        i += 2
        #Properties
        ...
        while payload_i < i+payload_length:
            ...
            payload_i += 2
            topic_filter = data[payload_i:payload_i+topic_filter_length].decode('utf-8')
            payload_i += topic_filter_length
            payloads[f'topic_filter {c}'] = topic_filter
            subscription_options = data[payload_i]
            payloads[f'subscription_options {c}'] = subscription_options
            payload_i += 1
            c += 1
    ...
    def parse_SUBACK(self, data: bytes, remaining_length):
        i=0
        packet_identifier = (data[i] << 8) | (data[i+1])
        i += 2
        #Properties
    ...
    def parse_UNSUBSCRIBE(self, data: bytes, remaining_length):
        i=0
        packet_identifier = (data[i] << 8) | (data[i+1])
        i += 2
        #Properties
    ...
    def parse_UNSUBACK(self, data: bytes, remaining_length):
        i=0
        packet_identifier = (data[i] << 8) | (data[i+1])
        i += 2
        #Properties
    ...
    def parse_PINGREQ(self):
        return {
                'PINGREQ packet'
                }
    
    #Работа с типом pingresp
    def parse_PINGRESP(self):
        return {
                'PINGRESP packet'
                }

    #Работа с типом disconnect
    def parse_DISCONNECT(self, data: bytes, remaining_length):
        if remaining_length == 0:
            return {
                'Type': 14,
                "Variable header": {
                    "DISCONNECT Reason Code": 0x00,
                    "Properties": None
                }
            }
        i = 0
    ...
    #Работа с типом auth
    def parse_AUTH(self, data: bytes, remaining_length):
        if remaining_length == 0:
            return {
                    'Type': 15,
                    "Variable header": {
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def deparse_fixed_header(self, packet):

        packet_type = packet['Type']

        if packet_type == 1: return self.deparse_CONNECT(packet)
        elif packet_type == 2: return self.deparse_CONNACK(packet)
        elif packet_type == 3: return self.deparse_PUBLISH(packet)
        elif packet_type == 4: return self.deparse_PUBACK(packet)
        elif packet_type == 5: return self.deparse_PUBREC(packet)
        elif packet_type == 6: return self.deparse_PUBREL(packet)
        elif packet_type == 7: return self.deparse_PUBCOMP(packet)
        elif packet_type == 8: return self.deparse_SUBSCRIBE(packet)
        elif packet_type == 9: return self.deparse_SUBACK(packet)
        elif packet_type == 10: return self.deparse_UNSUBSCRIBE(packet)
        elif packet_type == 11: return self.deparse_UNSUBACK(packet)
        elif packet_type == 12: return self.deparse_PINGREQ()
        elif packet_type == 13: return self.deparse_PINGRESP()
        elif packet_type == 14: return self.deparse_DISCONNECT(packet)
        elif packet_type == 15: return self.deparse_AUTH(packet)

    #Работа с типом connect
    def deparse_CONNECT(self, packet):
        #Fixed Header
        ...
        payload = bytearray()
            #Client id
        payload += len(packet['Payload']['ClientID']).to_bytes(2,'big') + packet['Payload']['ClientID'].encode()
            #Will payload
        if Will_Flag:
            #Will properties
    ...
    #Работа с типом connack
    def deparse_CONNACK(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом publish
    def deparse_PUBLISH(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом puback
    def deparse_PUBACK(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом pubrec
    def deparse_PUBREC(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом pubrel
    def deparse_PUBREL(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом pubcomp
    def deparse_PUBCOMP(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом subscribe
    def deparse_SUBSCRIBE(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом suback
    def deparse_SUBACK(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом unsubscribe
    def deparse_UNSUBSCRIBE(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        #Работа с типом suback
    #Работа с типом unsuback
    def deparse_UNSUBACK(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        return bytes(packet_bytes)
    #Работа с типом pingreq
    def deparse_PINGREQ(self):
        packet = bytearray()
        packet.append(0xC0)
        packet.append(0x0)
        return bytes(packet)
    #Работа с типом pingresp
    def deparse_PINGRESP(self):
        packet = bytearray()
        packet.append(0xD0)
        packet.append(0x0)
        return bytes(packet)
    #Работа с типом disconnect
    def deparse_DISCONNECT(self, packet):
        #Fixed Header
        fixed_header = bytearray()
        ...
        variable_header = bytearray()
            #Случай если reason code 0 и нет properties
        reason_code = packet["Variable header"].get('DISCONNECT Reason Code', 0)
        if reason_code == 0 and not packet["Variable header"].get("Properties"):
            body = variable_header
    ...
    def deparse_AUTH(self, packet):
        ...
        variable_header = bytearray()
            #Случай если reason code 0 и нет properties
        reason_code = packet["Variable header"].get('Authenticate Reason Code', 0)
        if reason_code == 0 and not packet["Variable header"].get("Properties"):
            body = variable_header
            fixed_header.append(0x00)
            packet_bytes = fixed_header + body
    ...
    def test(self):
        return MQTT_Protocol_Parse.deparse_fixed_header(self,test_packet)
    
    def test1(self):
        a = MQTT_Protocol_Parse.deparse_fixed_header(self,test_packet)
        b = MQTT_Protocol_Parse.parse_fixed_header(self,a)
        return b
...
test_packet =  {
    'Type': 15,
    "Variable header": {
        "Authenticate Reason Code": 24,
        "Properties": {
            "Authentication Method": "mqtt",
            "Authentication Data": b"\x01\x02\x03",
...
1 Эталон connect
{
    ...
    "Variable header": {
        ...
        "Properties": {
            ...
            "User Property": [("region", "eu"), ("platform", "linux")]
        }
    },
    "Payload": {
        ...
        "Will Topic": "system/lastwill",
        "Will Payload": b"critical disconnect",
        "User name": "admin",
        "Password": "secret"
    }
}
...
9 Эталон suback
{
    ...
    "Payload": {
        "Subscribe Reason Codes": [
            0,
            1
        ]
    }
}
10 Эталон unsubscribe
{
    'Type': 10,
    "Variable header": {
        "Packet Identifier": 1,
...
#todo:
#у suback были проблемы с payload там i вместо payload_i (!исправил, но вдруг еще где-то такое)