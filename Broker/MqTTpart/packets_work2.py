...
def parse_fixed_header(data: bytes):
    if not data:
    ...
    if control_packet_type == 1:
        ...
        return parse_CONNACK(data[1+b_count:(1+b_count)+val])
    elif control_packet_type == 3:
        return parse_PUBLISH(data[1+b_count:(1+b_count)+val], flags, val)
    elif control_packet_type == 4:
        return parse_PUBACK(data[1+b_count:(1+b_count)+val], val)
    elif control_packet_type == 5:
        return parse_PUBREC(data[1+b_count:(1+b_count)+val], val)
    elif control_packet_type == 6:
        return parse_PUBREL(data[1+b_count:(1+b_count)+val], val)
    elif control_packet_type == 7:
        return parse_PUBCOMP(data[1+b_count:(1+b_count)+val], val)
    elif control_packet_type == 8:
        return parse_SUBSCRIBE(data[1+b_count:(1+b_count)+val], val)
    elif control_packet_type == 9:
        return parse_SUBACK(data[1+b_count:(1+b_count)+val], val)
    elif control_packet_type == 10:
        return parse_UNSUBSCRIBE(data[1+b_count:(1+b_count)+val], val)
    elif control_packet_type == 11:
        return parse_UNSUBACK(data[1+b_count:(1+b_count)+val], val)
        ...
        return parse_DISCONNECT(data[1+b_count:(1+b_count)+val], val)
...
def parse_PUBLISH(data: bytes, flags, remaining_length):
...
def parse_PUBACK(data: bytes, remaining_length):
    i = 0
    packet_identifier = (data[i] << 8) | (data[i+1])
    i += 2
...
def parse_PUBREC(data: bytes, remaining_length):
    i = 0
    packet_identifier = (data[i] << 8) | (data[i+1])
    i += 2
    if (remaining_length == 2):
...
def parse_PUBREL(data: bytes, remaining_length):
    i = 0
    packet_identifier = (data[i] << 8) | (data[i+1])
    i += 2
    if (remaining_length == 2):
...
def parse_PUBCOMP(data: bytes, remaining_length):
    i = 0
    packet_identifier = (data[i] << 8) | (data[i+1])
    i += 2
    if (remaining_length == 2):
...
def parse_SUBSCRIBE(data: bytes, remaining_length):
    i = 0
    packet_identifier = (data[i] << 8) | (data[i+1])
    i += 2
    # Properties
...
def parse_SUBACK(data: bytes, remaining_length):
    i = 0
    packet_identifier = (data[i] << 8) | (data[i+1])
    i += 2
    # Properties
...
def parse_UNSUBSCRIBE(data: bytes, remaining_length):
    i = 0
    packet_identifier = (data[i] << 8) | (data[i+1])
    i += 2
    # Properties
...
def parse_UNSUBACK(data: bytes, remaining_length):
    i = 0
    packet_identifier = (data[i] << 8) | (data[i+1])
    i += 2
    # Properties
...
def parse_DISCONNECT(data: bytes, remaining_length):
...
def deparse_fixed_header(packet):

    packet_type = packet['Type']

    if packet_type == 1:
        return deparse_CONNECT(packet)
    elif packet_type == 2:
        return deparse_CONNACK(packet)
    elif packet_type == 3:
        return deparse_PUBLISH(packet)
    elif packet_type == 4:
        ...
        return deparse_SUBSCRIBE(packet)
    elif packet_type == 9:
        return deparse_SUBACK(packet)
    elif packet_type == 10:
        return deparse_UNSUBSCRIBE(packet)
    elif packet_type == 11:
        return deparse_UNSUBACK(packet)
    elif packet_type == 12:
...
def deparse_CONNECT(packet):
    ...
    # Keep-alive
    variable_header += packet["Variable header"]['Keep Alive'].to_bytes(
        2, 'big')
    ...
    if Will_Flag:
        ...
        # Will payload
        payload += len(packet['Payload']['Will Topic']).to_bytes(2,
                                                                 'big') + packet['Payload']['Will Topic'].encode()
        payload += len(packet['Payload']
                       ['Will Payload']).to_bytes(2, 'big')
        payload += packet['Payload']['Will Payload']
        # Username
    if User_Name_Flag:
        payload += len(packet['Payload']['User name']).to_bytes(2,
                                                                'big') + packet['Payload']['User name'].encode()
        # Password
    if Password_Flag:
        payload += len(packet['Payload']['Password']).to_bytes(2,
                                                               'big') + packet['Payload']['Password'].encode()
...
def deparse_CONNACK(packet):
    ...
    variable_header.append(
        packet["Variable header"]['Connect Acknowledge Flags'])
    # Connect Reason Code
    variable_header.append(
        packet["Variable header"]['Connect Reason Code'])
    # Properties UserProp номерки добавить
    connack_properties = {
        "Session Expiry Interval": lambda v: b'\x11' + v.to_bytes(4, 'big'),
        "Receive Maximum": lambda v: b'\x21' + v.to_bytes(2, 'big'),
...
def deparse_UNSUBSCRIBE(packet):
    # Fixed Header
    fixed_header = bytearray()
